...
from utils.helpers import get_deno_path, get_ffmpeg_path, sanitize_folder_name
...
class AutoClipperCore:
    ...
    def __init__(
        ...
        # Local Whisper settings — must be set BEFORE client creation
        self.local_whisper_settings = local_whisper_settings or {
            "enabled": False,
        ...
        if self.ai_providers:
            ...
            # Caption Maker client (Whisper API) — only create if NOT using local Whisper
            ...
            if not self.local_whisper_settings.get("enabled"):
                ...
                self.caption_client = None  # not needed in local mode
...
{transcript}"""
    ...
    def transcribe_full_video(self, video_path: str) -> str:
        """Transcribe full video audio using Whisper (local or API).
        
        Extracts audio from the video, compresses to mp3, splits into chunks
        if needed, and returns a transcript formatted like parse_srt output
        so find_highlights can consume it directly.
        
        Returns:
            str: Transcript with timestamps in SRT-like format:
                 [HH:MM:SS,mmm - HH:MM:SS,mmm] text
        """
        mode_str = "Local Whisper" if self.local_whisper_settings.get("enabled") else "Whisper API"
        self.log(f"[AI Transcription] Transcribing full video with {mode_str}...")
        
        # Check Caption Maker is configured (only needed for API mode)
        if not self.local_whisper_settings.get("enabled"):
            cm_config = self.ai_providers.get("caption_maker", {})
            if not cm_config.get("api_key"):
                raise Exception(
                    ...
                    "Or enable Local Whisper for free transcription."
                )
    ...
    def _whisper_transcribe_file(self, audio_path: str, time_offset: float = 0) -> list:
        """Route to local or API Whisper based on settings."""
        if self.local_whisper_settings.get("enabled"):
            return self._local_whisper_transcribe_file(audio_path, time_offset)
        else:
            return self._api_whisper_transcribe_file(audio_path, time_offset)
    
    def _api_whisper_transcribe_file(self, audio_path: str, time_offset: float = 0) -> list:
    ...
    # ------------------------------------------------------------------
    # Local Whisper (faster-whisper) methods
    # ------------------------------------------------------------------
    
    def _load_local_whisper(self):
        """Lazy load faster-whisper model. Downloads from HuggingFace on first use."""
        ...
        try:
            ...
            raise Exception(
                ...
                "Or disable Local Whisper in:\n"
                "Settings → AI API → Caption Maker"
            )
        
        settings = self.local_whisper_settings
        ...
        self.log(f"  [Local Whisper] Loading model: {model_size}")
        self.log(f"  [Local Whisper] Device: {device} | Compute: {compute_type}")
        self.log(f"  [Local Whisper] First time? Model akan otomatis di-download (~1-3GB)")
        ...
        self.log(f"  [Local Whisper] ✓ Model loaded!")
        return self._local_whisper_model
    
    def _local_whisper_transcribe_file(self, audio_path: str, time_offset: float = 0) -> list:
        """Transcribe a single audio file using local faster-whisper model.
        
        Returns same format as _api_whisper_transcribe_file:
        list of dicts with 'start', 'end', 'text' keys.
        """
        model = self._load_local_whisper()
        
        lang = self.subtitle_language if self.subtitle_language and self.subtitle_language != "none" else None
        
        file_size_mb = os.path.getsize(audio_path) / (1024 * 1024)
        self.log(f"    [Local Whisper] Transcribing {file_size_mb:.1f}MB audio...")
        ...
        if lang is None and hasattr(info, 'language'):
            self.log(f"    [Local Whisper] Detected language: {info.language} ({info.language_probability:.0%})")
        
        segments = []
        for seg in segments_gen:
            if self.is_cancelled():
                self.log(f"    ⚠️ Cancelled during local transcription")
                return segments
            segments.append({
                "start": seg.start + time_offset,
                "end": seg.end + time_offset,
                "text": seg.text.strip(),
                "words": [{"word": w.word, "start": w.start + time_offset, "end": w.end + time_offset}
                          for w in (seg.words or [])]
            })
        
        self.log(f"    [Local Whisper] ✓ Got {len(segments)} segments")
        return segments
    
    def _local_whisper_transcribe_words(self, audio_path: str):
        """Word-level transcription with local Whisper.
        
        Returns same shape as _whisper_transcribe_words_api:
        SimpleNamespace with .words, .segments, .text
        """
        from types import SimpleNamespace
        
        # Compress WAV → MP3 if needed (faster-whisper handles both, but smaller = faster)
        upload_path = audio_path
        mp3_tmp = None
        if audio_path.lower().endswith(".wav"):
            mp3_tmp = audio_path.rsplit(".", 1)[0] + "_local.mp3"
            cmd = [
                self.ffmpeg_path, "-y",
                "-i", audio_path,
                "-acodec", "libmp3lame",
            ...
            if result.returncode == 0 and os.path.exists(mp3_tmp):
                ...
                self.log("  [Caption] MP3 compression failed, using WAV as-is")
                mp3_tmp = None
        
        file_size_mb = os.path.getsize(upload_path) / (1024 * 1024)
        self.log(f"  [Caption] Local Whisper transcribing {file_size_mb:.2f}MB...")
        
        segments_list = self._local_whisper_transcribe_file(upload_path)
        ...
        # Build segments in same format expected by create_ass_subtitle_capcut
        segments_out = [{"start": s["start"], "end": s["end"], "text": s["text"]}
                        for s in segments_list]
        
        full_text = " ".join(s["text"] for s in segments_list)
        self.log(f"  [Caption] Local Whisper OK, text length: {len(full_text)}")
        self.log(f"  [Caption] Got {len(all_words)} words, {len(segments_out)} segments")
        
        return SimpleNamespace(words=all_words, segments=segments_out, text=full_text)
    
    def _whisper_transcribe_words(self, audio_path: str):
        """Route word-level transcription to local or API based on settings."""
        if self.local_whisper_settings.get("enabled"):
            return self._local_whisper_transcribe_words(audio_path)
        else:
